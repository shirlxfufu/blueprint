...
export const App: React.FC = () => {
  ...
  useEffect(() => {
    loadIcons();
    setManualOverrides(loadManualOverrides());
  }, []);
  ...
  const filteredIcons = useMemo(() => {
    let filtered = iconsWithOverrides;
    ...
    return filtered;
  }, [iconsWithOverrides, searchQuery, filterMode]);

  const handleToggleUnfilled = (iconName: string) => {
    const newOverrides = { ...manualOverrides };
    const currentOverride = newOverrides[iconName] || {};
    const currentValue = currentOverride.isUnfilled ?? false;
    newOverrides[iconName] = {
      ...currentOverride,
      isUnfilled: !currentValue,
      timestamp: Date.now()
    };
  ...
  const handleToggleMajorChange = (iconName: string) => {
    const newOverrides = { ...manualOverrides };
    const currentOverride = newOverrides[iconName] || {};
    const currentValue = currentOverride.hasMajorChange ?? false;
    newOverrides[iconName] = {
      ...currentOverride,
      hasMajorChange: !currentValue,
      timestamp: Date.now()
    };
  ...
  const handleExportCSV = () => {
    exportTaggedIconsCSV(iconsWithOverrides);
  };

  const handleExportHTML = () => {
    exportHTMLReport(iconsWithOverrides);
  };
    ...
    <div className={styles.appContainer}>
      ...
      <nav className={styles.filterBar}>
        <div className={styles.filterChips}>
          <div
            className={`${styles.filterChip} ${filterMode === 'all' ? styles.chipAll : ''}`}
      ...
      <main className={styles.mainContent}>
          ...
          <div className={styles.gridContainer}>
            {filteredIcons.map(icon => (
              <IconComparisonCard
                ...
                oldIconSvg={icon.oldIcon || null}
                newIconSvg={icon.newIconSvg || null}
            ...
            {/* Empty placeholder cards for visual consistency */}
            {filteredIcons.length > 0 && filteredIcons.length < 9 && (
              <>
                {Array.from({ length: Math.min(5, 9 - filteredIcons.length) }).map((_, i) => (
                  <div key={`placeholder-${i}`} className={styles.card} style={{ opacity: 0.5 - i * 0.1 }}>
        ...
        {!loading && filteredIcons.length === 0 && (
          <div style={{ textAlign: 'center', padding: '48px', maxWidth: '400px', margin: '0 auto' }}>
            <h3>No icons found</h3>
            <p>
              {searchQuery
                ? 'Try a different search query'
                : filterMode !== 'all'
                ? `No icons tagged as "${filterMode === 'outline' ? 'Outline Change' : 'Major Design Change'}"`
                : 'No icons available'}
            </p>
          </div>
        )}
