...
import { AbstractComponent, Utils as CoreUtils, DISPLAYNAME_PREFIX, HotkeysTarget, } from "@blueprintjs/core";
...
import { clampNumFrozenColumns, clampNumFrozenRows, compareChildren, getHotkeysFromProps, hasLoadingOption, isSelectionModeEnabled, } from "./tableUtils";
/**
 * Table component.
 *
 * @see https://blueprintjs.com/docs/#table/table
 */
export class Table extends AbstractComponent {
    ...
    render() {
        return _jsx(HotkeysTarget, { hotkeys: this.hotkeys, children: this.renderTableContents });
    }
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        const didChildrenChange = !compareChildren(Children.toArray(this.props.children), this.state.childrenArray);
        ...
        const didColumnWidthsChange = !Utils.compareSparseArrays(this.props.columnWidths ?? this.state.columnWidths, prevState.columnWidths);
        const didRowHeightsChange = !Utils.compareSparseArrays(this.props.rowHeights ?? this.state.rowHeights, prevState.rowHeights);
    ...
    renderMenu = (refHandler) => {
        ...
        return (
        // eslint-disable-next-line jsx-a11y/no-static-element-interactions
        _jsx("div", { className: classes, ref: refHandler, onMouseDown: this.handleMenuMouseDown, children: this.maybeRenderRegions(this.styleMenuRegion) }));
    };
    ...
    columnHeaderCellRenderer = (columnIndex) => {
        ...
        if (columnProps.name != null) {
            return _jsx(ColumnHeaderCell, { ...baseProps });
        }
        else {
            return _jsx(ColumnHeaderCell, { ...baseProps, name: Utils.toBase26Alpha(columnIndex) });
        }
    };
    renderColumnHeader = (refHandler, resizeHandler, reorderingHandler, showFrozenColumnsOnly = false) => {
        ...
        if (this.grid === null || this.locator === undefined || viewportRect === undefined) {
            ...
            return _jsx("div", { className: classes, ref: refHandler });
        }
    ...
    renderBody = (quadrantType, showFrozenRowsOnly = false, showFrozenColumnsOnly = false) => {
        ...
        return (_jsxs("div", { children: [_jsx(TableBody, { enableMultipleSelection: enableMultipleSelection, cellRenderer: this.bodyCellRenderer, focusedRegion: focusedRegion, focusMode: FocusedCellUtils.getFocusModeFromProps(this.props), grid: this.grid, loading: hasLoadingOption(loadingOptions, TableLoadingOption.CELLS), locator: this.locator, onCompleteRender: onCompleteRender, onFocusedRegion: this.handleFocus, onSelection: this.getEnabledSelectionHandler(RegionCardinality.CELLS), bodyContextMenuRenderer: bodyContextMenuRenderer, renderMode: this.getNormalizedRenderMode(), selectedRegions: selectedRegions, selectedRegionTransform: selectedRegionTransform, viewportRect: viewportRect, columnIndexStart: columnIndexStart, columnIndexEnd: columnIndexEnd, rowIndexStart: rowIndexStart, rowIndexEnd: rowIndexEnd, numFrozenColumns: showFrozenColumnsOnly ? numFrozenColumns : undefined, numFrozenRows: showFrozenRowsOnly ? numFrozenRows : undefined }), this.maybeRenderRegions(this.styleBodyRegion, quadrantType)] }));
    };
    isGuideLayerShowing() {
    ...
    validateGrid({ columnWidths, rowHeights } = {}) {
        if (this.grid == null || columnWidths !== undefined || rowHeights !== undefined) {
            ...
            this.grid = new Grid(rowHeights ?? this.state.rowHeights, columnWidths ?? this.state.columnWidths, gridBleed, defaultRowHeight, defaultColumnWidth);
    ...
    maybeRenderRegions(getRegionStyle, quadrantType) {
        ...
        const regionGroups = Regions.joinStyledRegionGroups(this.state.selectedRegions, this.props.styledRegionGroups ?? [], this.state.focusedRegion);
        return regionGroups.map((regionGroup, index) => {
            ...
            return (_jsx(RegionLayer, { className: classNames(regionGroup.className), regions: regionGroup.regions, regionStyles: regionStyles }, index));
        });
    }
    ...
    syncViewportPosition = ({ nextScrollLeft, nextScrollTop }) => {
        ...
        if (nextScrollLeft !== undefined || nextScrollTop !== undefined) {
            ...
            const nextViewportRect = new Rect(nextScrollLeft ?? viewportRect.left, nextScrollTop ?? viewportRect.top, viewportRect.width, viewportRect.height);
