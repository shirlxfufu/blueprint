...
class LocatorImpl {
    ...
    // =======
    getViewportRect() {
        return new rect_1.Rect(this.scrollContainerElement.scrollLeft || 0, this.scrollContainerElement.scrollTop || 0, this.scrollContainerElement.clientWidth, this.scrollContainerElement.clientHeight);
    }
    getWidestVisibleCellInColumn(columnIndex) {
        ...
        for (let i = 0; i < columnHeaderAndBodyCells.length; i++) {
            const contentWidth = utils_1.Utils.measureElementTextContent(columnHeaderAndBodyCells.item(i)).width;
     ...
     */
    hasVerticalOverflowOrExactFit(columnHeaderHeight = grid_1.Grid.MIN_COLUMN_HEADER_HEIGHT, viewportRect = this.getViewportRect()) {
     ...
     */
    hasHorizontalOverflowOrExactFit(rowHeaderWidth = grid_1.Grid.MIN_ROW_HEADER_WIDTH, viewportRect = this.getViewportRect()) {
        if (this.grid === undefined) {
            return false;
        }
    ...
    convertPointToCell(clientX, clientY) {
        const gridX = this.toGridX(clientX);
        const gridY = this.toGridY(clientY);
        const col = utils_1.Utils.binarySearch(gridX, this.grid.numCols - 1, this.convertCellIndexToClientX);
        const row = utils_1.Utils.binarySearch(gridY, this.grid.numRows - 1, this.convertCellIndexToClientY);
        return { col, row };
    }
    // Private helpers
    // ===============
