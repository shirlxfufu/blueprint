...
import { areFilmsEqual, filterFilm, renderFilm, TOP_100_FILMS } from "../../__examples__";
/**
 ...
 */
export function selectPopoverTestSuite(render, findPopover = wrapper => wrapper.find(`.${Classes.POPOVER}`), findTarget = wrapper => wrapper.find(`.${Classes.POPOVER_TARGET}`)) {
    const defaultProps = {
    ...
    describe("popoverProps functionality", () => {
        ...
        it("targetTagName allows customizing the target element", () => {
            ...
            assert.isTrue(anchorElement.exists(), `Expected to find popover target element with tag name '${targetTagName}'`);
            wrapper.detach();
        });
    });
}
