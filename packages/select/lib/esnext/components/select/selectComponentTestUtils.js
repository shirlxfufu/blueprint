...
import { areFilmsEqual, createFilm, createFilms, filterFilm, renderFilm, TOP_100_FILMS, } from "../../__examples__";
export function selectComponentSuite(render, findInput = wrapper => wrapper.find("input"), findItems = wrapper => wrapper.find("a")) {
    const testProps = {
    ...
    describe("keyboard", () => {
        it("arrow down invokes onActiveItemChange with next filtered item", () => {
            ...
            assert.equal(testProps.onActiveItemChange.lastCall.args[0].rank, 3);
        });
        it("arrow up invokes onActiveItemChange with previous filtered item", () => {
            const wrapper = render(testProps);
            findInput(wrapper).simulate("keydown", { key: "ArrowUp" });
            assert.equal(testProps.onActiveItemChange.lastCall.args[0].rank, 20);
        });
    ...
    describe("create", () => {
        const testCreateProps = {
            ...
            createNewItemRenderer: () => _jsx("textarea", {}),
            noResults: _jsx("address", {}),
        };
        beforeEach(() => {
        ...
        it("when createNewItemFromQuery returns an array, it should invoke onItemSelect once per each item in the array", () => {
            ...
            assert.equal(testCreateProps.onItemSelect.args[0][0].title, "non-existent film name", "should create and select first item");
            assert.equal(testCreateProps.onItemSelect.args[1][0].title, "second film name", "should create and select second item");
        });
        it("when create item is rendered, arrow down invokes onActiveItemChange with activeItem=null and isCreateNewItem=true", () => {
            ...
            findInput(wrapper).simulate("keydown", { key: "ArrowDown" });
            assert.equal(testProps.onActiveItemChange.lastCall.args[0].rank, TOP_100_FILMS[0].rank);
