...
export const Method = ({ className, value }) => {
    ...
    if (member === undefined) {
        ...
        if (isTsClass(classMember) && possibleClassMethod) {
            ...
            if (isTsMethod(classMethod)) {
                return _jsx(MethodTable, { className: className, data: classMethod });
            }
        }
        throw new Error(`Unknown @method ${value}`);
    }
    else if (isTsMethod(member)) {
        return _jsx(MethodTable, { className: className, data: member });
    }
    else {
